...
def _RBF(x1, x2, phi_name, c=1.0, alpha=1.0, r0=1.0):
    """
...
def _create_Afs(x : np.ndarray, a : np.ndarray, phi_name : np.ndarray) -> np.ndarray:
    '''
    ...
    A_fs[:1,:4] = a_transpose # columns 2 to 4 are x_a, y_a, z_a (tranpspose of input A matrix)
    ...
    for i in range(N_a): # for each row (N_a rows in total)
        for j in range(N_s): # columns 4 to end - compute RBFs
            ...
            phi = _RBF(x1=a_i, x2=s_j, phi_name=phi_name) # compute RBF phi_ai_sj using Eqn [12]
            A_fs[i,j+4] = phi


    return A_fs

def _create_Css(x : np.ndarray, phi_name : np.ndarray) -> np.ndarray:
    '''
...
def create_H(x : np.ndarray, a : np.ndarray, phi_name : str) -> np.ndarray:
    '''
